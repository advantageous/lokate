...
if [ 200 -eq $RESPONSE ]; then
	curl -X PUT -H "Content-Type: application/json" $MESOS_URL/$MARATHON_APP_ID -d @"build/mesos-deploy-staging.json"
else
	curl -X POST -H "Content-Type: application/json" $MESOS_URL -d @"build/mesos-deploy-staging.json"
fi
